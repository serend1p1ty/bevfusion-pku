...
class PointCloud(ABC):
    ...
    def from_file_multisweep(
        ...
        # Init.
        points = np.zeros(
            (cls.nbr_dims(), 0), dtype=np.float32 if cls == LidarPointCloud else np.float64
        )
        all_pc = cls(points)
...
class LidarSegPointCloud:
    ...
    def load_points(self, path: str) -> None:
        ...
        self.points = LidarPointCloud.from_file(
            path
        ).points.T  # [N, 4], where N is the number of points.
        if self.labels is not None:
            assert len(self.points) == len(
                self.labels
            ), "Error: There are {} points in the point cloud, " "but {} labels".format(
    ...
    def render(
        ...
        def draw_rect(selected_corners, color):
            ...
            for corner in selected_corners:
                axis.plot(
                    [prev[0], corner[0]], [prev[1], corner[1]], color=color, linewidth=linewidth
                )
                prev = corner
