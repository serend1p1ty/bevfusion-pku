...
class ResizeMultiViewImage(Resize):
    def __init__(
        self, img_scale=None, multiscale_mode="range", ratio_range=None, keep_ratio=True, **kwargs
    ):
            ...
            for key_im in results["img"]:
                if self.keep_ratio:
                    img, scale_factor = mmcv.imrescale(
                        key_im, results["scale"], return_scale=True, backend=self.backend
                    )
...
class NormalizeMultiViewImage(Normalize):
    def __init__(self, mean, std, to_rgb=True):
        super(NormalizeMultiViewImage, self).__init__(mean=mean, std=std, to_rgb=to_rgb)
...
class RandomScaleImageMultiViewImage(object):
    ...
    def __call__(self, results):
        ...
        results["img"] = [
            mmcv.imresize(img, (x_size, y_size), return_scale=False) for img in results["img"]
        ]
