...
class SSTInputLayerV2(nn.Module):
    ...
    def forward(self, voxel_feats, voxel_coors, batch_size=None):
        ...
        voxel_info = self.drop_voxel(
            voxel_info, 2)  # voxel_info is updated in this function
    ...
    def get_pos_embed(self, inds_dict, coors_in_win, feat_dim, dtype):
        ...
        # [num_tokens, c]
        if ndim == 3:
            pos_embed_2d = torch.cat([embed_x, embed_y, embed_z],
                                     dim=-1).to(dtype)
        else:
        ...
        if gap > 0:
            assert ndim == 3
            padding = torch.zeros((pos_embed_2d.size(0), gap),
                                  dtype=dtype,
    ...
    def get_key_padding_mask(self, ind_dict):
        ...
        key_padding = torch.ones(
            (num_all_voxel, 1)).to(ind_dict['voxel_drop_level'].device).bool()
