...
class SSTInputLayer(nn.Module):
    ...
    def forward(self, voxel_feat, coors):
        ...
        if self.shuffle_voxels:
            ...
            for k, tensor in voxel_info.items():
                if isinstance(tensor,
                              torch.Tensor) and len(tensor) == num_voxel:
                    voxel_info[k] = tensor[shuffle_inds]

        voxel_info = self.window_partition(coors, voxel_info)
        voxel_info = self.get_voxel_keep_inds(voxel_info, len(
            self.shifts_list))  # voxel_info is updated in this function
        ...
        flat2win_inds_list = [
            self.get_flat2win_inds(voxel_info[f'batch_win_inds_shift{i}'],
                                   voxel_info[f'voxel_drop_level_shift{i}'])
            for i in range(len(self.shifts_list))
        ]

        if self.debug:
            coors_3d_dict_shift0 = flat2window(
                coors, voxel_info['voxel_drop_level_shift0'],
                flat2win_inds_list[0], self.drop_info)
    ...
    def get_inner_win_inds(self, win_inds):
        ...
        template[end_pos_mask] = (num_tokens_each_win -
                                  1) * -1  #[1,1,-2, 0, 1,-1]
        ...
        if self.debug:
            ...
            random_win = unique_sort_inds[random.randint(
                0,
                len(unique_sort_inds) - 1)]
    ...
    def get_inner_win_inds_slow(self, win_inds):
        ...
        for ind in unique_win_inds:
            mask = win_inds == ind
            num = mask.sum().item()
            inner_inds[mask] = torch.arange(
                num, dtype=win_inds.dtype, device=win_inds.device)
        assert (inner_inds >= 0).all()
        return inner_inds
    ...
    def window_partition(self, coors, voxel_info):
        ...
        max_num_win_x = int(np.ceil((bev_shape_x / win_shape_x)) +
                            1)  # plus one here to meet the needs of shift.
        max_num_win_y = int(np.ceil((bev_shape_y / win_shape_y)) +
                            1)  # plus one here to meet the needs of shift.
        ...
        for i in range(len(shifts_list)):
            ...
            shifted_coors_x = coors[:, 3] + (
                win_shape_x - shift_x if shift_x > 0 else 0)
            shifted_coors_y = coors[:, 2] + (
                win_shape_y - shift_y if shift_y > 0 else 0)

            win_coors_x = shifted_coors_x // win_shape_x
            win_coors_y = shifted_coors_y // win_shape_y
            batch_win_inds = coors[:,
            ...
            voxel_info[f'coors_in_win_shift{i}'] = torch.stack(
                [coors_in_win_x, coors_in_win_y], dim=-1)
    ...
    def make_continuous_inds(self, inds):
        ...
        canvas = -torch.ones(
            (max_origin_inds + 1, ), dtype=dtype, device=device)
