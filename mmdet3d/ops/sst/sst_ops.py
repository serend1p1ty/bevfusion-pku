...
def get_flat2win_inds(batch_win_inds, voxel_drop_lvl, drop_info, debug=True):
    ...
    for dl in drop_info:  # dl: short for drop level
        ...
        if debug:
            assert inner_win_inds.max(
            ) < max_tokens, f'Max inner inds({inner_win_inds.max()}) larger(equal) than {max_tokens}'
            ...
            assert max_ind >= (
                num_windows - 1
            ) * max_tokens, f'max_ind({max_ind}) less than lower bound({(num_windows-1) * max_tokens})'
...
def window2flat(feat_3d_dict, inds_dict):
    ...
    all_flat_feat = torch.zeros((num_all_voxel, feat_dim),
                                device=device,
                                dtype=dtype)
    check_feat = -torch.ones(
        (num_all_voxel, ), device=device, dtype=torch.long)
...
def get_flat2win_inds_v2(batch_win_inds,
                         voxel_drop_lvl,
                         drop_info,
                         debug=True):
...
def get_inner_win_inds(win_inds):
    ...
    )  #sort_inds is like [0,0,0, 1, 2,2] -> [0,1, 2, 0, 0, 1]
...
def get_window_coors(coors, sparse_shape, window_shape, do_shift):
    ...
    # print(coors, 'coors')
    print(shifted_coors_z, 'shift_coors')
    print(win_coors_z, 'win_coors_z')
    # if len(window_shape) == 2:
...
def make_continuous_inds(inds):
    ...
    canvas[unique_inds] = torch.arange(
        num_valid_inds, dtype=dtype, device=device)
...
class SRATensor(object):
    ...
    def drop_and_partition(self, batching_info, key):
        ...
        self.set_reuse(
            key, False, 'drop_level', drop_lvl_s0, allow_override=False)
        self.set_reuse(
            key,
            False,
            'batch_win_inds',
        ...
        self.set_reuse(
            key, False, 'coors_in_win', coors_in_win_s0, allow_override=False)

        self.set_reuse(
            key, True, 'drop_level', drop_lvl_s1, allow_override=False)
        self.set_reuse(
            key,
            True,
            'batch_win_inds',
            ...
            allow_override=False)
        self.set_reuse(
            key, True, 'coors_in_win', coors_in_win_s1, allow_override=False)
    ...
    def setup(self, batching_info, key, window_shape, temperature):
        ...
        transform_info_s1 = self.get_reuse(
            key, False, 'transform_info', allow_missing=False)
        transform_info_s2 = self.get_reuse(
            key, True, 'transform_info', allow_missing=False)

        drop_lvl_s1 = self.get_reuse(
            key, False, 'drop_level', allow_missing=False)
        drop_lvl_s2 = self.get_reuse(
            key, True, 'drop_level', allow_missing=False)

        mask_s1 = self.get_key_padding_mask(transform_info_s1, drop_lvl_s1,
                                            batching_info,
                                            self._features.device)
        mask_s2 = self.get_key_padding_mask(transform_info_s2, drop_lvl_s2,
                                            batching_info,
                                            self._features.device)
        ...
        coors_in_win_s1 = self.get_reuse(
            key, False, 'coors_in_win', allow_missing=False)
        coors_in_win_s2 = self.get_reuse(
            key, True, 'coors_in_win', allow_missing=False)

        feat_dim = self._features.size(1)
        pos_s1 = self.get_pos_embed(transform_info_s1, coors_in_win_s1,
                                    drop_lvl_s1, batching_info, feat_dim,
                                    temperature, self._features.dtype)
        pos_s2 = self.get_pos_embed(transform_info_s2, coors_in_win_s2,
                                    drop_lvl_s2, batching_info, feat_dim,
                                    temperature, self._features.dtype)
    ...
    def window_tensor(self, do_shift):
        ...
        # def flat2window(feat, voxel_drop_lvl, flat2win_inds_dict, drop_info):
        window_tensor_dict = flat2window(self._features, drop_level,
                                         transform_info, self.batching_info)
        key_padding_mask = self.get_reuse(
            key, do_shift, 'mask', allow_missing=False)
    ...
    def update(self, window_tensor_dict):
        ...
        transform_info = self.get_reuse(self.key, self.shifted,
    ...
    def compute_and_add_transform_info(self, batching_info, key, do_shift):
        batch_win_inds = self.get_reuse(
            key, do_shift, 'batch_win_inds', allow_missing=False)
        drop_level = self.get_reuse(
            key, do_shift, 'drop_level', allow_missing=False)
        transform_info = self.get_transform_info(batch_win_inds, drop_level,
                                                 batching_info)
        self.set_reuse(
            key,
            do_shift,
    ...
    def get_transform_info(self, batch_win_inds, voxel_drop_lvl, drop_info):
        ...
        for dl in drop_info:
            ...
            flat2window_inds_dict[dl] = (flat2window_inds,
                                         torch.where(dl_mask))
    ...
    def window_partition(self, do_shift):
        ...
        batch_win_inds = self.indices[:,
                                      0] * max_num_win_per_sample + win_coors_x * max_num_win_y + win_coors_y
    ...
    @torch.no_grad()
    def get_voxel_keep_inds(self, batch_win_inds_s0, batch_win_inds_s1,
                            drop_info):
        '''
        ...
        keep_mask_s0, drop_lvl_s0 = self.drop_single_shift(
            batch_win_inds_s0, drop_info)
        ...
        keep_mask_s1, drop_lvl_s1 = self.drop_single_shift(
            batch_win_inds_s1, drop_info)
    ...
    def get_key_padding_mask(self, transform_info, voxel_drop_level,
                             batching_info, device):

        num_all_voxel = len(voxel_drop_level)
        key_padding = torch.ones((num_all_voxel, 1)).to(device).bool()

        window_key_padding_dict = flat2window(key_padding, voxel_drop_level,
                                              transform_info, batching_info)
    ...
    def get_pos_embed(self, transform_info, coors_in_win, voxel_drop_level,
                      batching_info, d_model, pos_temperature, dtype):
        '''
        '''
        # [N,]
        ...
        window_pos_emb_dict = flat2window(pos_embed_2d, voxel_drop_level,
                                          transform_info, batching_info)
