...
def test_flip():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="RandomFlip", flip_ratio=[0.7, 0.8], direction=["horizontal", "vertical"]
        )
        build_from_cfg(transform, PIPELINES)
    ...
    transform = dict(
        type="RandomFlip", flip_ratio=0.9, direction=["horizontal", "vertical", "diagonal"]
    )
    flip_module = build_from_cfg(transform, PIPELINES)
...
def test_random_crop():
    ...
    transform = dict(
        type="RandomCrop", crop_type="relative", crop_size=(0.3, 0.7), allow_negative_crop=True
    )
    transform_module = build_from_cfg(transform, PIPELINES)
    results_transformed = transform_module(copy.deepcopy(results))
    h, w = results_transformed["img_shape"][:2]
    ...
    transform = dict(
        type="RandomCrop", crop_type="absolute", crop_size=(1, 2), allow_negative_crop=True
    )
    transform_module = build_from_cfg(transform, PIPELINES)
    results_transformed = transform_module(copy.deepcopy(results))
    h, w = results_transformed["img_shape"][:2]
    ...
    transform = dict(
        type="RandomCrop", crop_type="absolute_range", crop_size=(1, 20), allow_negative_crop=True
    )
    transform_module = build_from_cfg(transform, PIPELINES)
    results_transformed = transform_module(copy.deepcopy(results))
    h, w = results_transformed["img_shape"][:2]
...
def test_random_center_crop_pad():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="RandomCenterCropPad", crop_size=(-1, 0), test_mode=False, test_pad_mode=None
        )
        build_from_cfg(transform, PIPELINES)
...
def test_multi_scale_flip_aug():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="MultiScaleFlipAug", img_scale=[1333, 800], transforms=[dict(type="Resize")]
        )
        build_from_cfg(transform, PIPELINES)
...
def test_cutout():
    ...
    transform = dict(
        type="CutOut", n_holes=(2, 4), cutout_shape=[(10, 10), (15, 15)], fill_in=(255, 255, 255)
    )
    cutout_module = build_from_cfg(transform, PIPELINES)
    cutout_result = cutout_module(copy.deepcopy(results))
    assert cutout_result["img"].sum() > img.sum()
