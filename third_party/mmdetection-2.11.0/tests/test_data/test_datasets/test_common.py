...
@pytest.mark.parametrize("config_path", ["./configs/_base_/datasets/voc0712.py"])
...
def test_dataset_evaluation():
    ...
    coco_cfg = dict(
        type="CocoDataset", ann_file=[fake_json_file, fake_json_file], classes=("car",), pipeline=[]
    )
    ...
    # build concat dataset through concatenated ann_file
    concat_cfg = dict(
        type="CustomDataset", ann_file=[fake_pkl_file, fake_pkl_file], classes=("car",), pipeline=[]
    )
...
@pytest.mark.parametrize(
    "classes, expected_length", [(["bus"], 2), (["car"], 1), (["bus", "car"], 2)]
)
