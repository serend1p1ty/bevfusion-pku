...
def _encoder_layer_forward(self, x, pos=None, attn_mask=None, key_padding_mask=None):
    norm_cnt = 0
    inp_residual = x
...
def test_multihead_attention(
    embed_dims=8, num_heads=2, dropout=0.1, num_query=5, num_key=10, batch_size=1
):
    ...
    out = module(query, key, None, residual, query_pos, key_pos, None, key_padding_mask)
    assert out.shape == (num_query, batch_size, embed_dims)

    # set attn_mask
    attn_mask = torch.rand(num_query, num_key) > 0.5
    out = module(query, key, key, residual, query_pos, key_pos, attn_mask, key_padding_mask)
    assert out.shape == (num_query, batch_size, embed_dims)
...
def test_transformer_encoder_layer(
    embed_dims=8, num_heads=2, feedforward_channels=8, num_key=10, batch_size=1
):
    x = torch.rand(num_key, batch_size, embed_dims)
    # test invalid number of order
    with pytest.raises(AssertionError):
        order = ("norm", "selfattn", "norm", "ffn", "norm")
        module = TransformerEncoderLayer(embed_dims, num_heads, feedforward_channels, order=order)
    ...
    @patch("mmdet.models.utils.TransformerEncoderLayer.forward", _encoder_layer_forward)
    @patch("mmdet.models.utils.FFN.forward", _ffn_forward)
    @patch("mmdet.models.utils.MultiheadAttention.forward", _multihead_attention_forward)
    def test_order():
        module = TransformerEncoderLayer(embed_dims, num_heads, feedforward_channels)
        out = module("input")
...
def test_transformer_decoder_layer(
    embed_dims=8, num_heads=2, feedforward_channels=8, num_key=10, num_query=5, batch_size=1
):
    query = torch.rand(num_query, batch_size, embed_dims)
    # test invalid number of order
    with pytest.raises(AssertionError):
        order = ("norm", "selfattn", "norm", "multiheadattn", "norm", "ffn", "norm")
        module = TransformerDecoderLayer(embed_dims, num_heads, feedforward_channels, order=order)
    ...
    module = TransformerDecoderLayer(embed_dims, num_heads, feedforward_channels)
    ...
    out = module(
        query, memory, memory_pos, query_pos, memory_key_padding_mask=memory_key_padding_mask
    )
    assert out.shape == (num_query, batch_size, embed_dims)
    ...
    @patch("mmdet.models.utils.TransformerDecoderLayer.forward", _decoder_layer_forward)
    @patch("mmdet.models.utils.FFN.forward", _ffn_forward)
    @patch("mmdet.models.utils.MultiheadAttention.forward", _multihead_attention_forward)
    def test_order():
        module = TransformerDecoderLayer(embed_dims, num_heads, feedforward_channels)
        ...
        assert (
            out == "input_selfattn(residual=input)_norm0_multiheadattn"
        ...
        out = module("input", "memory")
        assert (
            out == "input_norm0_selfattn(residual=input)_norm1_"
...
def test_transformer_encoder(
    num_layers=2, embed_dims=8, num_heads=2, feedforward_channels=8, num_key=10, batch_size=1
):
    module = TransformerEncoder(num_layers, embed_dims, num_heads, feedforward_channels)
    assert not module.pre_norm
...
def test_transformer(
    num_enc_layers=2, num_dec_layers=2, embed_dims=8, num_heads=2, num_query=5, batch_size=1
):
    ...
    # pre_norm
    module = Transformer(embed_dims, num_heads, num_enc_layers, num_dec_layers, pre_norm=True)
    hs, mem = module(x, mask, query_embed, pos_embed)
    assert hs.shape == (1, batch_size, num_query, embed_dims)
    assert mem.shape == (batch_size, embed_dims, height, width)
    ...
    # return_intermediate
    module = Transformer(
        embed_dims, num_heads, num_enc_layers, num_dec_layers, return_intermediate_dec=True
    )
    hs, mem = module(x, mask, query_embed, pos_embed)
