...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dims, num_heads, dropout=0.0):
        ...
        assert embed_dims % num_heads == 0, (
            "embed_dims must be " f"divisible by num_heads. got {embed_dims} and {num_heads}."
        )
    ...
    def forward(
        ...
        if key_pos is not None:
            key = key + key_pos
        out = self.attn(
            query, key, value=value, attn_mask=attn_mask, key_padding_mask=key_padding_mask
        )[0]
...
class FFN(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_fcs - 1):
            layers.append(
                nn.Sequential(
                    Linear(in_channels, feedforward_channels), self.activate, nn.Dropout(dropout)
                )
            )
