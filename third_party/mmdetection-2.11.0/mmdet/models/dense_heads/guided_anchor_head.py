...
class GuidedAnchorHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        self.feature_adaption = FeatureAdaption(
            self.in_channels, self.feat_channels, kernel_size=3, deform_groups=self.deform_groups
        )
    ...
    def get_anchors(
        self, featmap_sizes, shape_preds, loc_preds, img_metas, use_loc_filter=False, device="cuda"
    ):
    ...
    def _ga_shape_target_single(
        ...
        assign_result = self.ga_assigner.assign(
            approxs, squares, self.approxs_per_octave, gt_bboxes, gt_bboxes_ignore
        )
    ...
    def loss_shape_single(
        ...
        pred_anchors_ = self.anchor_coder.decode(bbox_anchors_, bbox_deltas_, wh_ratio_clip=1e-6)
    ...
    def loss(
        ...
        shape_targets = self.ga_shape_targets(
            approxs_list, inside_flag_list, squares_list, gt_bboxes, img_metas
        )
        ...
        for i in range(len(loc_preds)):
            loss_loc = self.loss_loc_single(
                loc_preds[i], loc_targets[i], loc_weights[i], loc_avg_factor=loc_avg_factor
            )
        ...
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_shape=losses_shape, loss_loc=losses_loc
        )

    @force_fp32(apply_to=("cls_scores", "bbox_preds", "shape_preds", "loc_preds"))
    def get_bboxes(
        self, cls_scores, bbox_preds, shape_preds, loc_preds, img_metas, cfg=None, rescale=False
    ):
