...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            ...
            conv_pred = nn.Conv2d(self.out_channels[i], self.num_anchors * self.num_attrib, 1)
    ...
    def get_bboxes(self, pred_maps, img_metas, cfg=None, rescale=False, with_nms=True):
        ...
        scale_factors = [img_metas[i]["scale_factor"] for i in range(pred_maps_list[0].shape[0])]
        result_list = self._get_bboxes(pred_maps_list, scale_factors, cfg, rescale, with_nms)
        return result_list

    def _get_bboxes(self, pred_maps_list, scale_factors, cfg, rescale=False, with_nms=True):
        ...
        multi_lvl_anchors = self.anchor_generator.grid_anchors(featmap_sizes, device)
        ...
        for i in range(self.num_levels):
            ...
            if nms_pre_tensor > 0 and (
                torch.onnx.is_in_onnx_export() or conf_pred.shape[1] > nms_pre_tensor
            ):
        ...
        if deploy_nms_pre > 0 and torch.onnx.is_in_onnx_export():
            ...
            batch_mlvl_conf_scores = batch_mlvl_conf_scores[batch_inds, topk_inds]
        ...
        if with_nms and cfg.get("nms", None) is not None:
            det_results = []
            for (mlvl_bboxes, mlvl_scores, mlvl_conf_scores) in zip(
                batch_mlvl_bboxes, batch_mlvl_scores, batch_mlvl_conf_scores
            ):
                ...
                if conf_thr > 0 and (not torch.onnx.is_in_onnx_export()):
                    ...
                    conf_inds = mlvl_conf_scores.ge(conf_thr).nonzero(as_tuple=False).squeeze(1)
            ...
            det_results = [
                tuple(mlvl_bs)
                for mlvl_bs in zip(batch_mlvl_bboxes, batch_mlvl_scores, batch_mlvl_conf_scores)
            ]
        return det_results
    ...
    def loss(self, pred_maps, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        return dict(
            loss_cls=losses_cls, loss_conf=losses_conf, loss_xy=losses_xy, loss_wh=losses_wh
        )
