...
class SABLRetinaHead(BaseDenseHead):
    ...
    def __init__(
        ...
        square_anchor_generator=dict(
            type="AnchorGenerator", ratios=[1.0], scales=[4], strides=[8, 16, 32, 64, 128]
        ),
        conv_cfg=None,
        norm_cfg=None,
    ...
    def _get_target_single(
        ...
        if len(pos_inds) > 0:
            ...
            ) = self.bbox_coder.encode(sampling_result.pos_bboxes, sampling_result.pos_gt_bboxes)
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        return dict(
            loss_cls=losses_cls, loss_bbox_cls=losses_bbox_cls, loss_bbox_reg=losses_bbox_reg
        )
