...
class PAAHead(ATSSHead):
    ...
    def loss(
        ...
        losses_cls = self.loss_cls(
            cls_scores, labels, labels_weight, avg_factor=max(num_pos, len(img_metas))
        )  # avoid num_pos=0
        if num_pos:
            ...
            losses_iou = self.loss_centerness(
                iou_preds[pos_inds_flatten], iou_target.unsqueeze(-1), avg_factor=num_pos
            )
    ...
    def get_pos_loss(
        self, anchors, cls_score, bbox_pred, label, label_weight, bbox_target, bbox_weight, pos_inds
    ):
    ...
    def paa_reassign(
        self, pos_losses, label, label_weight, bbox_weight, pos_inds, pos_gt_inds, anchors
    ):
    ...
    def _get_bboxes(
        ...
        for cls_score, bbox_pred, iou_preds, anchors in zip(
            ...
            iou_preds = iou_preds.permute(0, 2, 3, 1).reshape(batch_size, -1).sigmoid()
        ...
        for (mlvl_bboxes, mlvl_scores) in zip(batch_mlvl_bboxes, batch_mlvl_nms_scores):
            ...
            if self.with_score_voting and len(det_bbox) > 0:
                det_bbox, det_label = self.score_voting(
                    det_bbox, det_label, mlvl_bboxes, mlvl_scores, cfg.score_thr
                )
