...
class RPNHead(RPNTestMixin, AnchorHead):
    ...
    def _get_bboxes(
        self, cls_scores, bbox_preds, mlvl_anchors, img_shapes, scale_factors, cfg, rescale=False
    ):
        ...
        nms_pre_tensor = torch.tensor(cfg.nms_pre, device=cls_scores[0].device, dtype=torch.long)
        for idx in range(len(cls_scores)):
            ...
            if nms_pre_tensor > 0:
                ...
                if torch.onnx.is_in_onnx_export():
                    ...
                    nms_pre = torch.where(
                        scores_shape[1] < nms_pre_tensor, scores_shape[1], nms_pre_tensor
                    )
        ...
        result_list = []
        for (mlvl_proposals, mlvl_scores, mlvl_ids) in zip(
            batch_mlvl_proposals, batch_mlvl_scores, batch_mlvl_ids
        ):
            ...
            dets, keep = batched_nms(mlvl_proposals, mlvl_scores, mlvl_ids, cfg.nms)
