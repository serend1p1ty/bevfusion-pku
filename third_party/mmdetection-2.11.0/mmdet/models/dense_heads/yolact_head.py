...
class YOLACTHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        self.conv_reg = nn.Conv2d(self.feat_channels, self.num_anchors * 4, 3, padding=1)
        self.conv_coeff = nn.Conv2d(
            self.feat_channels, self.num_anchors * self.num_protos, 3, padding=1
        )
    ...
    def get_bboxes(self, cls_scores, bbox_preds, coeff_preds, img_metas, cfg=None, rescale=False):
        ...
        for img_id in range(len(img_metas)):
            ...
            coeff_pred_list = [coeff_preds[i][img_id].detach() for i in range(num_levels)]
    ...
    def _get_bboxes_single(
        ...
        for cls_score, bbox_pred, coeff_pred, anchors in zip(
            cls_score_list, bbox_pred_list, coeff_preds_list, mlvl_anchors
        ):
    ...
    def loss(self, segm_pred, gt_masks, gt_labels):
        ...
        for idx in range(num_imgs):
            ...
            if segm_targets is None:
                loss = self.loss_segm(
                    cur_segm_pred, torch.zeros_like(cur_segm_pred), torch.zeros_like(cur_segm_pred)
                )
            else:
                loss = self.loss_segm(
                    cur_segm_pred, segm_targets, avg_factor=num_imgs * mask_h * mask_w
                )
    ...
    def get_targets(self, segm_pred, gt_masks, gt_labels):
        ...
        with torch.no_grad():
            downsampled_masks = F.interpolate(
                gt_masks.unsqueeze(0), (mask_h, mask_w), mode="bilinear", align_corners=False
            ).squeeze(0)
...
class YOLACTProtonet(nn.Module):
    ...
    def _init_layers(self):
        ...
        for num_channels, kernel_size in zip(self.proto_channels, self.proto_kernel_sizes):
            if kernel_size > 0:
                layer = nn.Conv2d(in_channels, num_channels, kernel_size, padding=kernel_size // 2)
            else:
    ...
    def loss(self, mask_pred, gt_masks, gt_bboxes, img_meta, sampling_results):
        ...
        for idx in range(num_imgs):
            ...
            mask_targets = self.get_targets(cur_mask_pred, cur_gt_masks, pos_assigned_gt_inds)
            if num_pos == 0:
                loss = cur_mask_pred.sum() * 0.0
            elif mask_targets is None:
                loss = F.binary_cross_entropy(
                    cur_mask_pred, torch.zeros_like(cur_mask_pred), torch.zeros_like(cur_mask_pred)
                )
            else:
                cur_mask_pred = torch.clamp(cur_mask_pred, 0, 1)
                loss = (
                    F.binary_cross_entropy(cur_mask_pred, mask_targets, reduction="none")
    ...
    def get_seg_masks(self, mask_pred, label_pred, img_meta, rescale):
        ...
        mask_pred = (
            F.interpolate(
                mask_pred.unsqueeze(0), (img_h, img_w), mode="bilinear", align_corners=False
            ).squeeze(0)
            > 0.5
        )
