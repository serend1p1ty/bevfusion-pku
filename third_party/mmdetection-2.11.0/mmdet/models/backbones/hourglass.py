...
class HourglassModule(nn.Module):
    ...
    def __init__(
        self, depth, stage_channels, stage_blocks, norm_cfg=dict(type="BN", requires_grad=True)
    ):
        ...
        self.low1 = ResLayer(
            BasicBlock, cur_channel, next_channel, cur_block, stride=2, norm_cfg=norm_cfg
        )
...
class HourglassNet(nn.Module):
    ...
    def __init__(
        ...
        self.remap_convs = nn.ModuleList(
            [
                ConvModule(feat_channel, cur_channel, 1, norm_cfg=norm_cfg, act_cfg=None)
                for _ in range(num_stacks - 1)
            ]
        )
