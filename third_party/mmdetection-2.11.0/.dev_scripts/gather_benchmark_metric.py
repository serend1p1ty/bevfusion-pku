...
def parse_args():
    ...
    parser.add_argument("benchmark_json", type=str, help="json path of benchmark models")
    ...
    for config in model_configs:
        ...
        if osp.exists(result_path):
            ...
            if osp.exists(osp.join(result_path, ckpt_path)):
                log_json_path = list(sorted(glob.glob(osp.join(result_path, "*.log.json"))))[-1]
                ...
                if args.excel:
                    if "AR@1000" in model_performance:
                        ...
                        metrics = (
                            f'{model_performance["bbox_mAP"]}/' f'{model_performance["segm_mAP"]}'
                        )
                    else:
