...
modelzoo_dict = {
    "configs/faster_rcnn/faster_rcnn_r50_fpn_1x_coco.py": {"bbox": 0.374},
    "configs/mask_rcnn/mask_rcnn_r50_fpn_1x_coco.py": {"bbox": 0.382, "segm": 0.347},
    "configs/rpn/rpn_r50_fpn_1x_coco.py": {"AR@1000": 0.582},
}
...
def parse_args():
    ...
    parser.add_argument("json_out", help="the output json records test information like mAP")
    parser.add_argument(
        "--launcher",
...
def main():
    ...
    for model_family_dir in os.listdir(args.model_dir):
        for model in os.listdir(os.path.join(args.model_dir, model_family_dir)):
            ...
            assert os.path.isfile(cfg_path), f"{cfg_path} is not valid config path"
            cpt_path = os.path.join(args.model_dir, model_family_dir, model)
            result_dict[cfg_path] = cpt_path
            assert cfg_path in modelzoo_dict, f"please fill the " f"performance of cfg: {cfg_path}"
    ...
    try:
        ...
        if rank == 0:
            ...
            metrics = [m if m != "AR@1000" else "proposal_fast" for m in metrics]
