...
model = dict(
    ...
    train_cfg=dict(
        rcnn=dict(
            assigner=dict(pos_iou_thr=0.6, neg_iou_thr=0.6, min_pos_iou=0.6), sampler=dict(num=256)
        )
    ),
...
data = dict(
    ...
    val=dict(
        proposal_file=data_root + "proposals/ga_rpn_r50_fpn_1x_val2017.pkl", pipeline=test_pipeline
    ),
    test=dict(
        proposal_file=data_root + "proposals/ga_rpn_r50_fpn_1x_val2017.pkl", pipeline=test_pipeline
    ),
