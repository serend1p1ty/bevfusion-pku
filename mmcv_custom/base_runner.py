...
class BaseRunner(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if batch_processor is not None:
            if not callable(batch_processor):
                raise TypeError(
                    "batch_processor must be callable, " f"but got {type(batch_processor)}"
                )
            warnings.warn(
    ...
    @abstractmethod
    def save_checkpoint(
        self, out_dir, filename_tmpl, save_optimizer=True, meta=None, create_symlink=True
    ):
        pass
    ...
    def load_checkpoint(
        self, filename, map_location="cpu", strict=False, revise_keys=[(r"^module.", "")]
    ):
        return load_checkpoint(
            self.model, filename, map_location, strict, self.logger, revise_keys=revise_keys
        )
