...
class EpochBasedRunnerSuperAmp(EpochBasedRunnerAmp):
    ...
    def get_cand_arch(self, max_arch=False, min_arch=False):
        ...
        if self.search_neck:
            arch["panas_arch"] = [
                np.random.randint(self.panas_type) for i in range(self.panas_d_range[1])
            ]
            arch["panas_d"] = np.random.randint(self.panas_d_range[0], self.panas_d_range[1] + 1)
            # arch['panas_d'] = self.panas_d_range[1]
            arch["panas_c"] = (
                np.random.randint(self.panas_c_range[0], self.panas_c_range[1] + self.c_interval)
                // self.c_interval
                * self.c_interval
            )
            if max_arch:
        ...
        if self.search_head:
            if self.head_d_range:
                ...
                arch["head_step"] = np.random.randint(
                    self.head_d_range[0], self.head_d_range[1] + 1
                )
                if max_arch:
                    arch["head_step"] = self.head_d_range[1]
                if min_arch:
