    ...
    with open(log_json_path, "r") as f:
        for line in f.readlines():
            ...
            if log_line["mode"] == "train" and max_memory <= log_line["memory"]:
                ...
                result_dict = {
                    key: log_line[key] for key in RESULTS_LUT[dataset] if key in log_line
                }
                if len(max_dict) == 0:
...
def main():
    ...
    for model in model_infos:
        ...
        model_name = model["config"].split("/")[-1].rstrip(".py") + "_" + model["model_time"]
        publish_model_path = osp.join(model_publish_dir, model_name)
        trained_model_path = osp.join(
            models_root, model["config"], "epoch_{}.pth".format(model["epochs"])
        )
        ...
        config_path = (
            osp.join("configs", config_path) if "configs" not in config_path else config_path
        )
