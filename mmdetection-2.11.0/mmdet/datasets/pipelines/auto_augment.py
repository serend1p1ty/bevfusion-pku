...
class Shear(object):
    ...
    def _shear_masks(
        self, results, magnitude, direction="horizontal", fill_val=0, interpolation="bilinear"
    ):
        """Shear the masks."""
        h, w, c = results["img_shape"]
        for key in results.get("mask_fields", []):
            ...
            results[key] = masks.shear(
                (h, w), magnitude, direction, border_value=fill_val, interpolation=interpolation
            )

    def _shear_seg(
        self, results, magnitude, direction="horizontal", fill_val=255, interpolation="bilinear"
    ):
        """Shear the segmentation maps."""
        for key in results.get("seg_fields", []):
            seg = results[key]
            results[key] = mmcv.imshear(
                seg, magnitude, direction, border_value=fill_val, interpolation=interpolation
            ).astype(seg.dtype)
    ...
    def __call__(self, results):
        ...
        self._shear_masks(
            results, magnitude, self.direction, fill_val=0, interpolation=self.interpolation
        )
        self._shear_seg(
            results,
            magnitude,
...
class Rotate(object):
    ...
    def _rotate_bboxes(self, results, rotate_matrix):
        ...
        for key in results.get("bbox_fields", []):
            ...
            coordinates = np.concatenate(
                (coordinates, np.ones((4, 1, coordinates.shape[2], 1), coordinates.dtype)), axis=1
            )  # [4, 3, nb_bbox, 1]
    ...
    def _rotate_seg(self, results, angle, center=None, scale=1.0, fill_val=255):
        ...
        for key in results.get("seg_fields", []):
            ...
            results[key] = mmcv.imrotate(seg, angle, center, scale, border_value=fill_val).astype(
                seg.dtype
            )
...
class Translate(object):
    ...
    def __init__(
        ...
        assert 0 <= level <= _MAX_LEVEL, (
            "The level used for calculating Translate's offset should be " "in range [0,_MAX_LEVEL]"
        )
    ...
    def _translate_img(self, results, offset, direction="horizontal"):
        ...
        for key in results.get("img_fields", ["img"]):
            img = results[key].copy()
            results[key] = mmcv.imtranslate(img, offset, direction, self.img_fill_val).astype(
                img.dtype
            )
