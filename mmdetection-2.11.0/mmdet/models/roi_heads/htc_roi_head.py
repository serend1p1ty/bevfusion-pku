...
class HybridTaskCascadeRoIHead(CascadeRoIHead):
    ...
    def _bbox_forward_train(
        self, stage, x, sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg, semantic_feat=None
    ):
    ...
    def forward_train(
        ...
        for i in range(self.num_stages):
            ...
            bbox_results = self._bbox_forward_train(
                i, x, sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg, semantic_feat
            )
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            ...
            if i < self.num_stages - 1:
                ...
                rois = torch.cat(
                    [
                        bbox_head.regress_by_class(
                            rois[i], bbox_label[i], bbox_pred[i], img_metas[i]
                        )
            ...
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta, semantic in zip(img_feats, img_metas, semantic_feats):
                    ...
                    mask_feats = self.mask_roi_extractor[-1](
                        x[: len(self.mask_roi_extractor[-1].featmap_strides)], mask_rois
                    )
                    if self.with_semantic:
