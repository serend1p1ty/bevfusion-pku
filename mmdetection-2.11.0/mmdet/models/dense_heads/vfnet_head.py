...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(FCOSHead, self).__init__(num_classes, in_channels, norm_cfg=norm_cfg, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.vfnet_reg_refine_dconv = DeformConv2d(
            self.feat_channels, self.feat_channels, self.dcn_kernel, 1, padding=self.dcn_pad
        )
    ...
    def _get_bboxes_single(
        ...
        for cls_score, bbox_pred, points in zip(cls_scores, bbox_preds, mlvl_points):
            ...
            scores = (
                cls_score.permute(1, 2, 0).reshape(-1, self.cls_out_channels).contiguous().sigmoid()
            )
    ...
    def _get_points_single(self, featmap_size, stride, dtype, device, flatten=False):
        ...
        if self.use_atss:
            points = (
                torch.stack((x.reshape(-1), y.reshape(-1)), dim=-1)
    ...
    def get_targets(
        self, cls_scores, mlvl_points, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore
    ):
    ...
    def get_atss_targets(
        self, cls_scores, mlvl_points, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None
    ):
