...
class RepPointsHead(AnchorFreeHead):
    ...
    def gen_grid_from_reg(self, reg, previous_boxes):
        ...
        intervel = (
            torch.linspace(0.0, 1.0, self.dcn_kernel).view(1, self.dcn_kernel, 1, 1).type_as(reg)
        )
    ...
    def centers_to_bboxes(self, point_list):
        ...
        for i_img, point in enumerate(point_list):
            ...
            for i_lvl in range(len(self.point_strides)):
                ...
                bbox_shift = (
                    torch.Tensor([-scale, -scale, scale, scale]).view(1, 4).type_as(point[0])
                )
    ...
    def _point_target_single(
        ...
        assign_result = assigner.assign(
            proposals, gt_bboxes, gt_bboxes_ignore, None if self.sampling else gt_labels
        )
    ...
    def get_bboxes(
        ...
        device = cls_scores[0].device
        bbox_preds_refine = [
            self.points2bbox(pts_pred_refine) for pts_pred_refine in pts_preds_refine
        ]
        num_levels = len(cls_scores)
        mlvl_points = [
            self.point_generators[i].grid_points(
                cls_scores[i].size()[-2:], self.point_strides[i], device
            )
            for i in range(num_levels)
        ]
        result_list = []
