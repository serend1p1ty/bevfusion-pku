...
def test_max_iou_assigner_with_empty_boxes_and_ignore():
    ...
    assign_result = self.assign(
        bboxes, gt_bboxes, gt_labels=gt_labels, gt_bboxes_ignore=gt_bboxes_ignore
    )
    assert len(assign_result.gt_inds) == 0
    assert tuple(assign_result.labels.shape) == (0,)
...
def test_center_region_assigner_with_ignore():
    ...
    assign_result = self.assign(
        bboxes, gt_bboxes, gt_bboxes_ignore=gt_bboxes_ignore, gt_labels=gt_labels
    )
...
def test_hungarian_match_assigner():
    ...
    assign_result = self.assign(bbox_pred, cls_pred, gt_bboxes, gt_labels, img_meta)
    ...
    # test iou mode
    self = HungarianAssigner(iou_cost=dict(type="IoUCost", iou_mode="iou", weight=1.0))
