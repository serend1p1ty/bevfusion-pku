...
from mmcv.tensorrt import TRTWraper, is_tensorrt_plugin_loaded, onnx2trt, save_trt_engine
...
def onnx2tensorrt(
    onnx_file, trt_file, input_config, verify=False, show=False, dataset="coco", workspace_size=1
):
    ...
    trt_engine = onnx2trt(
        onnx_model, opt_shape_dict, fp16_mode=False, max_workspace_size=max_workspace_size
    )
    ...
    if verify:
        ...
        if show:
            ...
            score_thr = 0.35
            imshow_det_bboxes(
                img.copy(), trt_boxes, trt_labels, CLASSES, score_thr=score_thr, win_name="TensorRT"
            )
            imshow_det_bboxes(
                img.copy(),
                ort_boxes,
        ...
        # Compare results
        np.testing.assert_allclose(ort_boxes, trt_boxes, rtol=1e-03, atol=1e-05)
        np.testing.assert_allclose(ort_labels, trt_labels)
        print("The numerical values are the same " + "between ONNXRuntime and TensorRT")
...
def parse_args():
    ...
    parser.add_argument(
        "--trt-file", type=str, default="tmp.trt", help="Filename of output TensorRT engine"
    )
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify the outputs of ONNXRuntime and TensorRT"
    )
    parser.add_argument(
        "--to-rgb", action="store_false", help="Feed model with RGB or BGR image. Default is RGB."
    )
    parser.add_argument(
        "--shape", type=int, nargs="+", default=[400, 600], help="Input size of the model"
    )
    parser.add_argument(
        "--mean",
        type=float,
