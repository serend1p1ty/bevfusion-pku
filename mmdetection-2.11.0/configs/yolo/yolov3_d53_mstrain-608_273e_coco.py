...
model = dict(
    ...
    neck=dict(
        type="YOLOV3Neck", num_scales=3, in_channels=[1024, 512, 256], out_channels=[512, 256, 128]
    ),
    bbox_head=dict(
        type="YOLOV3Head",
        num_classes=80,
...
train_pipeline = [
    ...
    dict(
        type="Expand", mean=img_norm_cfg["mean"], to_rgb=img_norm_cfg["to_rgb"], ratio_range=(1, 2)
    ),
